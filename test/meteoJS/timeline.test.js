QUnit.test("meteoJS.timeline helper functions", function (assert) {
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date(), []),
               -1, 'Leerer Such-Array');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2016-01-10T10:00:00'),
  ...
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2016-01-10T10:00:00'),
                                                       [new Date('2016-01-10T10:00:00'),
                                                        new Date('2016-06-10T10:00:00')]),
               0, '0 gefunden');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2016-06-10T10:00:00'),
                                                       [new Date('2016-01-10T10:00:00'),
                                                        new Date('2016-06-10T10:00:00')]),
               1, '0 gefunden');
});
QUnit.test("Empty object", function (assert) {
  ...
  var t = timeline.getSelectedTime();
...
QUnit.test("maxTimeGap: only 12z steps", function (assert) {
  ...
  var times = timeline.getTimes();
  assert.equal(times.length, 49, 'Anzahl Zeitschritte');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2017-02-06T12:00:00'),
                                                       times),
               0, '2017-02-06T12:00:00 gefunden');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2017-02-07T00:00:00'),
                                                       times),
               4, '2017-02-07T00:00:00 gefunden');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2017-02-08T09:00:00'),
                                                       times),
               15, '2017-02-08T09:00:00 gefunden');
  assert.equal(meteoJS.timeline._indexOfTimeInTimesArray(new Date('2017-02-11T03:00:00'),
                                                       times),
               37, '2017-02-11T03:00:00 gefunden');
});
QUnit.test("next()/add()", function (assert) {
  var timeline = new meteoJS.timeline();
  var times = [
    new Date('2016-01-01T00:00:00'),
...
QUnit.test("next()/add() mit Leerstellen", function (assert) {
  var timeline = new meteoJS.timeline({
    maxTimeGap: 3*3600
  });
  var times = [
  ...
  var t0 = new Date('2016-01-01T09:00:00');
  timeline.setTimesBySetID('Test', times);
...
QUnit.test("isLast/isFirst", function (assert) {
  var timeline = new meteoJS.timeline();
  var times = [
    new Date('2016-01-01T00:00:00'),
    new Date('2016-01-01T06:00:00'),
